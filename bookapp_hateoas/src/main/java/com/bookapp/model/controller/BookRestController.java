...
public class BookRestController {
	...
	// get all the books
	@GetMapping(path = "/book2", produces = MediaType.APPLICATION_JSON_VALUE)
	public CollectionModel<Book>   getAllBooksV2() {
		List<Book> books = bookService.getAllBooks();
		
		for(Book book: books) {
			Link link=linkTo(methodOn(BookRestController.class).getBookByIdV2(book.getId()))
					.withSelfRel();
			book.add(link);
		}
	...
	public EntityModel<Book> getBookByIdV2(@PathVariable(name = "id") int id) {
		// builder design pattern : design pattern
		// i have create a link that refer to itself : aka hello world of Hateoas

		Link link = linkTo(methodOn(BookRestController.class).getBookByIdV2(id)).withSelfRel();
